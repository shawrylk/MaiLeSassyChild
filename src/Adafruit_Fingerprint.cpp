...
uint8_t Adafruit_Fingerprint::getModel(void)
{
  ...
  // length:    N                       2 bytes;
  // data:      -                       N bytes;
  // sum:       type + length + data:   2 bytes;

  // Buffer size is 512 bytes, but packet's data size is maximum at 256 bytes
  // so we need to read 2 packets, each packet has size of 2 + 4 + 1 + 2 + 256 + 2 = 267 bytes
  // to store 2 packet we need buffer with size of 267 * 2 = 534 bytes
  ...
  while (index < 534)
  {
    // ignore header, address, type and length
    while (index < uindx)
    {
      ++index;
    }
    ...
    while (index < uindx)
    {
      fingerTemplate[index2] = bytesReceived[index];
      index++;
      index2++;
    }
    // ignore checksum
